...
async function resolveIsSealed(ix) {
  if (isGetAccount(ix) || isScript(ix)) {
    if (ix.block.isSealed == null && !ix.block.id && !ix.block.height) {
      ix.block.isSealed = !(await config.config().get("fcl.experimental.softFinality"));
    }
  }
  return ix;
}
...
const resolve = pipe([resolveIsSealed, resolveCadence, debug("cadence", (ix, log) => log(ix.message.cadence)), resolveComputeLimit, debug("compute limit", (ix, log) => log(ix.message.computeLimit)), resolveArguments, debug("arguments", (ix, log) => log(ix.message.arguments, ix.message)), resolveAccounts, debug("accounts", (ix, log, accts) => log(...accts(ix))), /* special */execFetchRef, /* special */execFetchSequenceNumber, resolveSignatures, debug("signatures", (ix, log, accts) => log(...accts(ix))), resolveFinalNormalization, resolveValidators, resolveVoucherIntercept, debug("resolved", (ix, log) => log(ix))]);
...
async function account(address) {
  ...
  return await send([getAccount(address), opts.isSealed != null && atLatestBlock(isSealed)], opts).then(decodeResponse);
}
