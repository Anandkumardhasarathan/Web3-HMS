...
export interface AptosNftSdk {
    ...
    transferV1Token(to: string, creator: string, collection: string, name: string, propertyVersion: string, amount: string): Promise<CommittedTransactionResponse>;
    burn(tokenAddress: string): Promise<CommittedTransactionResponse>;
}
export interface AptosBalanceSdk {
    getAptosBalance({ address }: {
        address: string;
    }): Promise<BigNumber>;
}
export interface AptosOrderSdk {
    ...
    acceptCollectionOfferV1(offer: string, tokenName: string | undefined, propertyVersion: string): Promise<CommittedTransactionResponse>;
...
export declare const supportedNetworks: readonly [AptosNetwork.MAINNET, AptosNetwork.TESTNET];
export type SupportedNetwork = (typeof supportedNetworks)[number];
export type AptosSdkConfig = {
    overrides?: Partial<AptosSettings>;
};
